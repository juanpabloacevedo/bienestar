					...
					<tr>
						...
						<td>                               
							...
							<td>
								...
								<form class="form-horizontal" method="GET" action="{{ route('usuarioactual',['id' => $user->id]) }}">
									{{ csrf_field() }}
									<div class="form-group">
										<div class="col-md-6 col-md-offset-4">
...
<form method="POST" action="{{ route('importcsv') }}" files="true" enctype="multipart/form-data">
	...
	<div class="file-field input-field">
		<div class="btn">
			...
			<input type="file" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" name="filecsv" onchange="changeInput(this)">
		</div>
		<div class="file-path-wrapper">
			<input class="file-path validate" type="text" placeholder="Upload one or more files">
		</div>
	</div>
	<button class="btn waves-effect waves-light orange" id="fileSubmit" type="submit" disabled="true" name="action">Cargar
	 ...
	 function change_status(user_id){
	 	$.ajax({
	 		...
	 		success:function(data){
	 			console.log(data);
	 			if(data.sancionado){
	 				M.toast({html: data.name+' '+'Sancionado'})
	 			}else{
	 				M.toast({html: data.name+' '+'Sin Sanción'})
	 			}
	 		},
	 ...
	 function change_status_activo(user_id){
	 	$.ajax({
	 		...
	 		success:function(data){
	 			console.log(data);
	 			if(data.activo){
	 				M.toast({html:data.name+' '+'Activo'})
	 			}else{
	 				M.toast({html: data.name+' '+'Inactivo'})
	 			}
	 		},
	 		error:function(error){
	 			console.log(error);
